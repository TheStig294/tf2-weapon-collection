local ROLE = {}
ROLE.nameraw = "redscout"
ROLE.name = "RED Scout"
ROLE.nameplural = "RED Scouts"
ROLE.nameext = "a RED Scout"
ROLE.nameshort = "rsc"
...
if SERVER then
    AddCSLuaFile()
    hook.Add("TTTPlayerRoleChanged", "TF2Scout_ClassChangeReset", function(ply, oldRole, newRole)
        if newRole == ROLE_REDSCOUT or newRole == ROLE_BLUSCOUT then
            ply:SetMaxJumpLevel(ply:GetMaxJumpLevel() + 1)
        end
        if oldRole == ROLE_REDSCOUT or oldRole == ROLE_BLUSCOUT then
            ply:SetMaxJumpLevel(ply:GetMaxJumpLevel() - 1)
        end
    end)
end
...
if CLIENT then
    hook.Add("TTTTutorialRoleText", "REDScout_TTTTutorialRoleText", function(role, _)
        if role == ROLE_REDSCOUT then
            local roleColor = GetRoleTeamColor(ROLE_TEAM_TRAITOR)
            local counterpartColour = GetRoleTeamColor(ROLE_TEAM_DETECTIVE)
            local html = "The " .. ROLE_STRINGS[ROLE_REDSCOUT] .. " is a <span style='color: rgb(" .. roleColor.r .. ", " .. roleColor.g .. ", " .. roleColor.b .. ")'>traitor</span> role who can move faster and jump an extra time!<br><br>To change to a different TF2 class, they can buy the 'Class Changer' item in their buy menu, which grants a full heal! However, <span style='color: rgb(" .. roleColor.r .. ", " .. roleColor.g .. ", " .. roleColor.b .. ")'>this costs a credit!</span><br><br>This role is the <span style='color: rgb(" .. roleColor.r .. ", " .. roleColor.g .. ", " .. roleColor.b .. ")'>traitor</span> equivalent to the " .. ROLE_STRINGS[ROLE_BLUSCOUT] .. ", which works exactly the same, except it is a <span style='color: rgb(" .. counterpartColour.r .. ", " .. counterpartColour.g .. ", " .. counterpartColour.b .. ")'>detective</span> role."
            return html
        end
    end)
end