...
if CLIENT then
	...
	SWEP.Icon = "vgui/ttt/weapon_ttt_tf2_bottle.png"
end
...
function SWEP:Think()
	if self.Attack and self.AttackTimer <= CurTime() then
		local owner = self:GetOwner()
		if not IsValid(owner) then return end
		...
		if SERVER and tr.Hit then
			if tr.Entity:IsNPC() or tr.Entity:IsPlayer() then
				owner:EmitSound("weapons/bottle_intact_hit_flesh1.wav")
			end

			if not (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) then
				owner:EmitSound("weapons/bottle_hit1.wav")
			end
		end
