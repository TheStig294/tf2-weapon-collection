...
SWEP.WorldModel = "models/weapons/v_models/v_fist_heavy.mdl"
...
if CLIENT then
     ...
     SWEP.Icon = "vgui/ttt/weapon_ttt_tf2_fists.png"
end
...
SWEP.Secondary.Sound = Sound("Weapon_Fist.Miss")
     ...
     if self.Attack == 1 and self.AttackTimer <= CurTime() then
          ...
          if SERVER and tr.Hit then
               if tr.Entity:IsNPC() or tr.Entity:IsPlayer() then
                    owner:EmitSound("Weapon_Fist.HitFlesh")
               end

               if not (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) then
                    owner:EmitSound("Weapon_Fist.HitWorld")
               end
          end
...
-- Only draw the third-person model if the fists are on the ground, the player's stance is enough to show they are using the fists
if CLIENT then
